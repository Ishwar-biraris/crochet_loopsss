...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sp-select-trigger:focus-visible { outline: 1px solid var(--dark); }
  .sp-select-menu {
    position: absolute;
    top: calc(100% + 2px);
    left: 0;
  ...
  .sp-select-option:hover { background: var(--line); }
  .sp-select-option.is-active { opacity: .45; pointer-events: none; }
  ...
  @media (max-width: 560px) { .sp-card { border-right: none !important; } }
  ...
  .sp-discover-btn:hover { background: var(--rose); letter-spacing: .28em; }
...
/* ─────────────────────────────────────────────────────
   PRODUCT CARD
───────────────────────────────────────────────────── */
const ProductCard = ({ product, index, CAT_ACCENT, CAT_LABEL }) => {
  const { name, category, price, oldPrice, badge, imageUrl, desc } = product;
  const saving = oldPrice ? Math.round(((oldPrice - price) / oldPrice) * 100) : null;

  // Support both Cloudinary imageUrl (new) and local imageName (old migrated products)
  ...
  return (
    <div className="sp-card" style={{ animationDelay: `${index * 0.05}s` }}>
      <div className="sp-img">
        <div
          className="sp-img-bg"
          style={{ background: `radial-gradient(ellipse at 40% 40%, #f0ebe2, #e8e0d4)` }}
        />
      ...
      <div className="sp-card-body">
        <span className="sp-cat-pill" style={{ color: CAT_ACCENT[category] || "#c05080" }}>
        ...
        <div className="sp-price-row">
          ...
          {oldPrice && <span className="sp-old-price">₹{oldPrice}</span>}
          {saving && <span className="sp-save">{saving}% off</span>}
        </div>
...
// Build CAT_ACCENT and CAT_LABEL dynamically from Firestore:
const CAT_ACCENT = Object.fromEntries(categories.map(c => [c.key, c.accent]));
const CAT_LABEL  = Object.fromEntries(categories.map(c => [c.key, c.singularLabel]));

  const [products, setProducts] = useState([]);       // ✅ live from Firestore
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "products"), (snap) => {
      const docs = snap.docs
        ...
        .filter(p => p.visible !== false); // hide products toggled off in admin
  ...
  <button
    className={`sp-cat-btn ${activeCategory === "all" ? "active" : ""}`}
    onClick={() => { setActiveCategory("all"); setSort("default"); }}
  >
    ALL
  </button>

  {/* Then dynamic categories from Firestore */}
  {categories.map(cat => (
    <button
      key={cat.key}
